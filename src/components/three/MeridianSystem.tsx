...
/** 一条经的连线（细管，颜色随五行） */
export function MeridianLine({ code, mirrored, dim, sex, radiusScale = 1, brightness = 1 }: {
  ...
  return (
    ...
    <mesh geometry={geo} renderOrder={2}>
      <meshBasicMaterial color={c} transparent opacity={Math.min(1, base * brightness)} depthWrite={false} />
    </mesh>
  );
}
 ...
 */
const QI_PER_LINE = 7;
export function QiFlow({ code, mirrored, speed, sex, size = 0.055, speedRef }: {
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial color={meridianColor(code)} size={size} transparent opacity={0.95}
